# System inventory & operational detail

On-demand reference. CLAUDE.md keeps only always-needed behavior; everything here is data, history or detail recoverable with a Read. Counts carry their own snapshot dates; recount before relying on one.
...
- Links `skills/commands/rules/docs/hooks/output-styles/workflows/scripts` into `~/.claude/` (authoritative folder list = `LINK_FOLDERS` in `scripts/sync-claude.ts`; rules link per-entry, with `test-policy.md` excluded from the global layer) (junctions on Windows — no admin; symlinks on macOS/Linux) and regenerates `~/.claude/settings.json` from `settings.global.json` deep-merged with `settings.machine.json` (gitignored, per-machine; hook groups are unioned per event and de-duplicated by matcher and command, so the overlay only adds machine-specific handlers), then validates the result with `claude doctor` ("Invalid settings" section) — a rejected file fails the sync and the backup is restored.
...
## System-prompt channel (multi-host)

**`.claude/output-styles/poneglyph.md` is the communication-law SSOT** — the primary document, edit THIS one; Claude Code reads it live through the folder symlink. It names no person ("the user"), so it can be reused. Contents: header (busy-lead brief) / Truth / Glance (order: short answer first, detail if it adds, optional `Aviso:` line, closing `ROBIN:` line with summary + next step — also the degradation canary logged by `security-gate.ts`) / Voice / Cost / References (Spanish codes: D decisiones · O opciones · H hallazgos · R riesgos · P preguntas · A acciones) / Limits (proactive in words, not in actions) / Examples (5 pairs). Prompts to other models follow `prompt-design` Context 3, and delegation and consultation prompts carry a `[REPLY FORMAT]` line so Codex/Grok answer the Lead in plain English. `system-prompts/poneglyph-sp.md` is the body-only twin **GENERATED by sync-claude** (frontmatter stripped, no marker — clean body) consumed by the grok symlink, sync-codex's concat, and compare.ts — never edit it by hand; after editing the style, re-run sync-claude (twin) + sync-codex (AGENTS.md). Compare harness: `.claude/evals/compare.ts` (presets + `--probes` adherence battery).
...
| Orca (agents it spawns) | Inherits `~/.claude/settings.json` outputStyle — **decision: NO append in Argumentos** (double-load measured useless) | live | `bun run doctor` carries it as the `Orca style append (sync-orca)` row; `bun .claude/scripts/sync-orca.ts` runs the same check standalone. Informative either way: 🟢 when no append is configured, 🟡 double-load warning if one appears |
...
|---|---|---|---|
| **Plain prompt** (`haz X`) | Normal turn | Lead triages complexity → invokes phase skills / `/flow-lifecycle` as needed | `skill-activation.ts` injects `Skill()` on keyword match **+** Lead self-matches via `agent-routing §05` on non-trivial work |
| **`/flow-lifecycle <task>`** | The **router** — feature lifecycle orchestrator | Deterministic: triages mode → runs phases 1-5 with hard gates, **invoking each phase skill explicitly** | Wired by `/flow-lifecycle` itself (phase→skill is hard-coded in the command); hook skips `/flow-lifecycle` (no double-routing) |
| **`/goal <condition>`** | Built-in **persistence loop** (Stop hook) — keeps the Lead working until the condition holds. NOT a router | None by itself — the Lead routes each turn via the always-loaded CLAUDE.md core | Hook skips it (like every slash command). `/goal` runs as the **Lead**, not a fresh-context agent: it already has the always-loaded routing core and can invoke any `Skill()` itself — it needs no injected hint. Whether the Lead *actually* loads the relevant skill on a work turn is a discretionary call (no enforcement); for guaranteed skill use, wrap the work in `/flow-lifecycle` |
| **`/expert-role <name>`** | Persona-framing over existing skills | Composes skills for the role; Lead still triages | Hook skips it (role self-composes); Lead matches within the persona |
...
Canonical per-turn checklist: `agent-routing` skill §1.
...
`minimal|standard|full` modes were removed; every phase runs and a justified skip
...
Full template and propagation model: `agent-routing/references/06-context-arch-h.md`.

## Key rules mapping (historical — old rule → current location)

| Old rule | Current location |
|---|---|
| `lead-orchestrator.md` | `.claude/skills/agent-routing/SKILL.md` |
| `orchestration-checklist.md` | `agent-routing/references/01-verification.md` |
| `prompt-scoring.md` | `prompt-design` skill |
| `complexity-routing.md` | `agent-routing/references/03-complexity-routing.md` |
| `agent-selection.md` | `agent-routing/references/04-agent-selection.md` |
| `context-management.md` | `agent-routing/references/06-context-arch-h.md` |
...
**Default to a skill.** Reach for a command only for (a) a script entrypoint (`sync-poneglyph.ts`) or (b) a pure prompt-macro that orchestrates skills with `$ARGUMENTS`/`allowed-tools` (`flow-lifecycle`, `expert-role`). Commands and user-invocable skills can accept arguments. Model invocation depends on host policy and skill metadata; keyword hints are advisory. Live commands: `flow-lifecycle`, `expert-role`, `sync-poneglyph` — everything else is a skill.
...
| `docs/` | This file + `research-rigor.md` + `auxiliary-skills-matrix.md` | on-demand references |
...
| `evals/` | Golden-prompt regression harness: deterministic graders + runner + real-failure cases (30 cases counted on 2026-09-13; recount `cases.jsonl` after changes). Tracked, NOT synced. Run per meta-config change | live |
| `scripts/` | `doctor.ts` — one-table health check: sync status (Claude + Codex + Grok), `claude plugin validate`, suite, budget ratchet, privacy grep, sessions-today by model, **context shape 7d** (`bun run doctor`) · `usage-profile.ts` + `lib/*` — SHAPE of the token spend from `~/.claude/projects/**/*.jsonl` (context per message, share above the 200k ceiling, top-session share, tool output by tool and Bash family); `bun run usage [--days N] [--cap N]`; feeds the doctor's Context row · `lib/host-config.ts` — one context/effort policy for every host (Claude `autoCompactWindow`/`effortLevel`, Codex `model_auto_compact_token_limit`/`model_reasoning_effort`, Grok `auto_compact_threshold_percent`/`default_reasoning_effort`), consumed by `sync-codex`/`sync-grok` · `budget.ts` + `lib/budget.ts` + `lib/budget-snapshot.json` — always-loaded token budget with a **0 %** ratchet enforced by `__tests__/budget.test.ts` (`bun run budget`, `--update` after a ratified size decision) · `flow-state.ts` + `lib/flow-contract.ts` — verified lifecycle transitions, disjoint review decisions and recoverable artifact projection; contract: `docs/flow-contract.md` · `gate.ts` — deterministic validation of what an agent CLAIMS it did (artifacts, delta-vs-baseline diff cross-check, verdict self-consistency, suite, protected paths, self-integrity); ported from disler's task-pipeline `gates.py` with delta-not-absolute-state, patterns derived from the user/global/project settings union, and a self-integrity check. Tracked and **synced** (sync-trap class closed: synced components instruct `bun .claude/scripts/...`) | live |
| `learned/` | Runtime per-machine (gitignored) | runtime |
| `ccstatusline/` | Statusline module wired via settings (synced to `~/.config/ccstatusline/`) | live |
| `config/` | `cost-budget.json` — phantom config nothing read | **deleted** |
| `data/`, `agent-memory/` | Telemetry remnants / empty dir | **deleted** |
| `plans/_archive/` | Closed/abandoned plans (gitignored, on disk only). `_archive/` has **zero functional dependents** — only historical prose cites archived plans by name. `plans/` retains active and closed plan records plus templates and research; status comes from each state record, not directory presence | archive |
...
Schema findings (refreshed 2026-07-08 against the live settings profile — now `settings.global.json` — + CC changelog): `minimumVersion` EXISTS and gates the version (the live value is whatever `settings.global.json` says — 2.1.228 at the last check; the floor was first set at 2.1.198, the release fixing conditional rules loaded via symlinked paths, which the whole global layer depends on — and the generated `~/.claude/settings.json` lags the repo until `sync-claude --execute` runs again); `fallbackModel` EXISTS since CC 2.1.166 and IS configured in settings.global.json (cascade of 2); `requiredMinimumVersion`/`requiredMaximumVersion` are **managed settings** for org policy (changelog 2.1.163) — they do NOT supersede personal `minimumVersion` (resolved).

Activation/observability hooks (event verified in official hooks docs 2026-06-10): `skill-activation.ts` (UserPromptSubmit) injects explicit `Skill(<name>)` instructions on PRECISE keyword match (multi-word phrase, or ≥2 distinct single-words; the old length-based tier measured a 2/54 honor rate) — the deterministic activation layer, best-effort per issue #17277. `instructions-loaded.ts` (InstructionsLoaded, async) logs every CLAUDE.md/rules load to `.claude/learned/instructions-loaded.log` — grep it to verify load layers instead of assuming.

`post-compact.ts` and its only input, the `CLAUDE_LEAD_MODE` env var, were removed: `instructions-loaded.log` shows CLAUDE.md and the always-on rules reloaded on the `compact` trigger, so the re-injected reminder duplicated the always-loaded layer. CLAUDE.md is the single owner of the git and spawn gates; skills, commands and the output style keep one-line pointers.
...
Oriol ratified retrospectives 035 and 036 during remediation planning. Their
state and document projections now record closure. This does not extend the
original pilot evidence or establish global activation.
