...
**Default to a skill.** Reach for a command only for (a) a script entrypoint (`sync-poneglyph.ts`) or (b) a pure prompt-macro that orchestrates skills with `$ARGUMENTS`/`allowed-tools` (`flow`, `role`). Commands and user-invocable skills can accept arguments. Model invocation depends on host policy and skill metadata; keyword hints are advisory. Live commands: `flow`, `role`, `sync-poneglyph` — everything else is a skill.
...
| Skills | 28 | 14 | **29** (source count 2026-09-13) | 6 phase skills + `drillme` + `html-report` (003); 031: `best-of-n` + `project-onboard` CUT, `decision-stress-test` MERGED into `decide` (heavy tier), `escalate`→`unstuck`, `codex-consult`→`consult` (multi-model), +`pr-review` (new); `skill-advisor` restored in 023, auto-invoke-the-obvious since 031 |
...
| Slash commands | 10 | 4 | **3** (source count 2026-09-13) | `flow`, `sync-poneglyph`, `role` (decide/explain-changes were thin command wrappers → pruned; they remain as skills) |
