...
| Orca (agents it spawns) | Inherits `~/.claude/settings.json` outputStyle — **decision 2026-08-18: NO append in Argumentos** (double-load measured useless) | live | `bun run doctor` carries it as the `Orca style append (sync-orca)` row; `bun .claude/scripts/sync-orca.ts` runs the same check standalone. Informative either way: 🟢 when no append is configured, 🟡 double-load warning if one appears |
...
| `prompt-scoring.md` | `prompt-design` skill (post-2026-05-28) |
