...
const CODEX_HINT_LINES: ReadonlyMap<string, string> = new Map([
  [FLOW_HINT_LINE, "Feature-shaped task → consider $flow-lifecycle — the full lifecycle (Skill flow: scope→plan→test-plan→build→review→retro). Wide scope → on Codex the 400k ceiling is a launch-time flag, not an in-session command: start with `codex -c model_auto_compact_token_limit=400000` (default ceiling 200k, plan 037)."],
