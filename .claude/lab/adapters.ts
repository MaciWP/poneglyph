...
export interface Telemetry { terminal: boolean; text: string; model: string | null; usage: Record<string, number> | null; apiEquivalentUsd: number | null }
const empty = (): Telemetry => ({terminal:false,text:"",model:null,usage:null,apiEquivalentUsd:null});
...
export function commandFor(host: Host, binary: string, model: string, prompt: string): string[] {
 ...
 return [binary,"-p",prompt,"--model",model,"--output-format","stream-json","--verbose","--no-session-persistence","--permission-mode","dontAsk","--allowedTools","Read","Write","Edit","Glob","Grep","Bash"];
}
export function decode(host: Host, raw: string): Telemetry {
 try {
  ...
  if (host === "grok") {
   ...
   return {terminal:true,text,model:typeof message.model==="string"?message.model:null,usage:usage(message.usage),apiEquivalentUsd:null};
  }
  if (host === "codex") {
   ...
   return {terminal:true,text:texts.at(-1),model:events.find(e=>typeof e.model==="string")?.model??null,usage:usage(events.at(-1).usage),apiEquivalentUsd:null};
  }
  ...
  return {terminal:true,text:result.result,model:result.modelUsage && Object.keys(result.modelUsage).length===1?Object.keys(result.modelUsage)[0]:null,usage:usage(result.usage),apiEquivalentUsd:number(result.total_cost_usd)?result.total_cost_usd:null};
 } catch { return empty(); }
}
