...
Verify: Parse native JSONL, retain model/profile identity and native hook trust. Run focused tests before implementation and after changes. Run the final project gates on the assembled change.
Ask first: Native model execution needs explicit model and budget; other choices follow the approved design.

## Acceptance criteria
Parse native JSONL, retain model/profile identity and native hook trust.
