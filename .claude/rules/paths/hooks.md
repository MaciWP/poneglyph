...
|-------|------|-------------|--------------------|
| PreToolUse | Before tool | Unreliable (#6305) | `headless-model-gate.ts` (matcher `Bash`, plan 033) — denies a `claude -p`/`--print` without `--model`, or with a Fable/Opus model (`--model`/`--fallback-model`) and no `--allow-expensive`, and a repo headless script (`evals/run.ts`, `compare.ts`, `probe-activation.ts`) with an expensive `--model`. Best-effort by contract: the braces are `scripts/lib/headless.ts`, which gives every script a cheap default on its own. Cause: 82 headless sessions on 2026-09-03, 44 Opus + 29 Fable |
