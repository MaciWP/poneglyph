...
For supervised Orca workers, load `orca-team`'s execution reference and the
...
2. Invoke `Skill('troubleshooting')` if the error type is non-obvious (cascading failures, retry storms, timeouts, transient errors).
