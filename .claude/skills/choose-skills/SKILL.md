...
- On demand: the user asks "qué skills aplican" / "propón skills" / "drillme-clarify de skills".
...
4. **Ratify** — present the shortlist to the human via `AskUserQuestion`:
   ...
   - Below the options (in the accompanying text), list the near-miss candidates DISCARDED with a
     one-line motive each ("descartada Y — matchea 'valida' pero es la fase 4 de /flow-lifecycle, no aplica").
