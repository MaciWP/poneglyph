...
- [When to escalate beyond drillme-clarify](#when-to-escalate-beyond-drillme)
...
- 5 iterations without clarity → drillme-clarify can't resolve this alone; escalate to `compare-and-decide` (heavy tier) or external investigation.
...
| Over-applying | Use 5-whys on a 1-line change | If decision is trivial, drillme-clarify itself shouldn't fire |
...
Patterns can be combined within one drillme-clarify:
...
Cap: combining is for high-stakes decisions. For routine drillme-clarify, stick to one pattern + canonical 4 categories.

## When to escalate beyond drillme-clarify

If after applying 1-2 complementary patterns the decision still doesn't crystallize → drillme-clarify has reached its ceiling. Options:

1. **`compare-and-decide` (heavy tier)**: spawn 5-12 perspectives in parallel + cross-debate + synthesis + vote.
...
Drillme closes gaps by asking the user. When the blocker is a genuine disagreement or the decision still won't crystallize after a full sweep, that is honest signal to escalate to a multi-voice debate (`compare-and-decide` (heavy tier)), not to keep re-asking.
