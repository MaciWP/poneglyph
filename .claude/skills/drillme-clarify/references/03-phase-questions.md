...
description: Phase-specific question banks for the poneglyph 5-phase workflow. Loaded by drillme-clarify when auto-detecting a phase from .claude/plans/{NNN}-{slug}/ artefacts.
---
...
## Loading rule for drillme-clarify

When `$ARGUMENTS` is empty and a phase is auto-detected (see SKILL.md §Workflow Step 1), drillme-clarify appends the relevant phase bank to the output **after** the canonical 4-category questions. Coverage report should show both:

```markdown
Coverage: <categories examined> + <relevant phase questions resolved>; gaps: <none or specific gaps>.
```
...
Smell signal: if drillme-clarify loaded a phase bank but the questions feel irrelevant to the user's actual concern → the auto-detection caught a stale artefact. Tell the user:
...
These banks are the **minimum** sweep per phase, not the maximum. Under the exhaustive model (SKILL.md), drillme-clarify also sweeps the lateral / improvement categories from `01-catalog-socratic.md` (partial failures, retries, security surface, migration, observability, cost…) whenever a gap there would change the decision. The phase bank identifies phase-critical topics to examine; the lateral sweep catches the gaps the requester didn't mention. Both are gap-gated — no padding.
