...
name: quality-check
description: How to evaluate answers to drillme-clarify questions — concretion criteria, iteration protocol, smell signals.
---

# Quality check for drillme-clarify answers
...
When the user (or Lead) answers drillme-clarify questions and asks for iteration, each answer must pass a quality check before being marked closed. The check prevents "ceremonia": questions that get fake-closed with vague answers and the decision proceeds on hot air.
...
When the user requests iteration after first drillme-clarify:
...
|---|---|---|
| User answers all questions in <30 seconds, all Concrete | Either the context was trivial (calibration was off — should have skipped drillme-clarify) OR user is rubber-stamping | If first: skip drillme-clarify next time for similar context. If second: dig — re-drill with `[failure]` focus |
| Same Evasive pattern across all answers ("depende"/"probably") | User is in low-engagement mode; drillme-clarify is producing ceremony | Stop. Ask: "¿Qué necesitas para responder con concreción?" |
...
| Answers reveal disagreement with another stakeholder | Drillme reached its ceiling | Escalate to `compare-and-decide` (heavy tier) or surface to the disagreeing stakeholder |
...
| Iterating until forced agreement | Escalating follow-ups until user just says "yes, fine" | If user is fatigued, drillme-clarify has failed; pause |
