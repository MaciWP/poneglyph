...
description: Why drillme exists — the Tenth Man principle, the doctrine it operationalizes and the research backing selective clarification.
---
...
A typo surfaces no gaps, so drillme runs and closes with zero questions; an ambiguous feature surfaces many, so it asks until they are gone. The gate is binary and generation-executable; there is no graduated calibration ("N questions for trivial, M for architectural").
