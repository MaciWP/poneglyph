...
description: |
  ...
  Úsala cuando: tasks/ aprobado + oracle de Fase 2.5 aprobado + HU pendiente en state.json, "build", "implementa", "ejecuta", "construye", "siguiente HU", tras /flow-test-plan y antes de /flow-review.
metadata:
  keywords: >
...
When the current prompt has a live Orca Dispatch under an approved `orca-team`
team, read its [worker contract](../orca-team/references/coordination.md).
...
> "Each HU closes intra-fase before the next one. Smallest diff that satisfies the AC + drillme-clarify." (Commandment V — simple by default; Commandment IV — blocking gates per HU)

Discipline: **atomicity + honest test/validation closure + inline execution** (one HU = one unit of work in the main session; "context isolation" is never a reason to spawn — `agent-routing` P1/P2). Rationale: `references/01-history.md`.

## When to use

| Trigger | Example |
|---|---|
| `tasks/` approved + `tests.md`/`validations.md` approved + HUs pending in `state.json` | After `/flow-test-plan` closes Phase 2.5 |
| User invokes `/flow-build` (next pending HU) or `/flow-build US{id}` (specific HU) | Direct invocation |
| `flow-test-plan` skill closes and triggers Phase 3 invocation | Auto-chain from Phase 2.5 |
...
| All HUs in `state.json` already completed | Phase 4 (`/flow-review`) instead |
...
|---|---|
| `/flow-build US{id}` | Execute that exact HU. If already completed in `state.json` → ask user (re-run? abort?). |
| `/flow-build` (no arg) | First HU in `state.json.us_pending` whose `depends_on` are all in `state.json.us_completed`. If none → check if a previously failed HU is in `us_pending_blocked` and escalate. |
...
| HU is "create extension" (new skill/hook/rule/MCP/plugin) | Lead invokes `harness-config` first; then inline | Meta-context requires `harness-config` consultation |
...
> A single HU is NEVER a reason to spawn. Fan-out is decided at the WAVE level (≥4 independent HUs), not per-HU — and it is user opt-in (Workflow). See `agent-routing` §Step 1 spawn decision tree + `references/04-agent-selection.md` §Workflow wiring.
...
5. **Lessons pass**: `Skill(lessons-learned)` — the cross-repo guards plus the `references/<stack>-*.md` matching the HU's stack (Django, React, …). Mandatory when the HU touches code recovered from a stash or an old branch (lesson G3).
...
3. If suite reveals breakage in unrelated code → diagnose with `troubleshooting` skill.
```
...
> Skill-to-skill invocation is probabilistic. If `drillme-clarify` does not auto-fire and a real doubt blocks progress, the Lead invokes `/drillme-clarify "<concrete doubt — HU US{N}>"` manually before closing the HU.

### Step 8 — Verify (blocking gate per HU)

Run `changes-verify` before changing completion state. Record all required checks on the final diff:
...
- If ANY of the above fails → invoke `troubleshooting` skill, retry per `error-recovery.md` budget. After 2 retries → escalate to user.
...
Next HU available: US{M} (depends_on satisfied)
  → /flow-build US{M} para ejecutar
  → /flow-build sin args para auto-seleccionar
  → /flow-review si todas las HUs cerradas (Phase 4)
```
...
The `builder` agent was cut in feature 008 (1 agent forbidden; "context isolation" is not a reason). A single HU runs inline; a ≥4-HU parallel wave may fan out via `Workflow` with the `default` subagent (`agent-routing` spawn tree). History: `references/01-history.md`.

## Auxiliary skills

Wiring and manual fallbacks for this phase (drillme-clarify and troubleshooting must fire on every HU; code-quality and harness-config conditional): `.claude/docs/auxiliary-skills-matrix.md` §Fallbacks per phase. Skill-to-skill invocation is probabilistic (issue #59968) — when an auxiliary does not fire, apply its fallback row.
...
| Signal | Adaptation |
|---|---|
| HU trivial (1 file, known pattern, no ambiguity) | Skip exhaustive Glob; 1-2 examples max; reduced drillme-clarify (most-relevant question only) |
| HU touches business-critical code AND `test-policy.md` = `business-critical` | Strict red→green; no shortcuts; full the relevant canonical drillme-clarify bank |
...
- **Edge 5** — User invokes `/flow-build` but `state.json` says Phase 3 already complete: ask if re-run a specific HU or transition to Phase 4.
...
- ⚠️ If `troubleshooting` retries exceed budget on the same HU → escalate; do not loop silently.
...
| V | Smallest diff that satisfies AC; over-engineering caught by drillme-clarify Q3 |
...
| IX | Meta-extensions go through `harness-config` skill consultation (extensible meta-system) |
