...
name: flow-plan
description: |
  Plan técnico a partir de un spec.md aprobado (Fase 2 del workflow `/flow-lifecycle`). Descompone el alcance en historias de usuario (HUs) atómicas con un DAG explícito de dependencias, tras investigación obligatoria (Context7 + WebFetch + Grep del proyecto). Produce tasks/index.md + N tasks/US{N}.md. Honra test-policy.md por nodo. Invoca flow-test-plan (Fase 2.5) al cerrar.
  Úsala cuando: existe spec.md aprobado y hace falta descomposición técnica, "tareas", "roadmap", "descomponer", "HU", "atomizar", "DAG", o /flow-plan. Evita la keyword suelta "plan" (colisiona con el modo plan de Claude Code).
metadata:
  keywords: >
    Keywords - flow-plan, tareas, roadmap, descomponer, HU, atomizar, DAG, dependencies,
...
Translates an approved `spec.md` (output of Phase 1 `flow-scope` skill) into a Validated Execution Graph: atomic HUs + DAG of real dependencies + research-backed decisions. The deliverable is `tasks/index.md` + N `tasks/US{N}.md` ready for the human hard gate 2->3.
...
Three obligatory verifications BEFORE writing any HU: project context (Glob/Grep), external doc (Context7), atomicity (drillme-clarify). Rationale and legacy migration: `references/07-history.md`.
...
| User invokes `/flow-plan` or mentions "tareas", "roadmap", "descomponer" | direct invocation |
...
| Anti-trigger | Why |
|---|---|
| No approved `spec.md` exists | Phase 1 first — invoke `flow-scope` skill |
| `spec.md` has `status: draft` (not approved) | Hard gate 1->2 not closed — escalate to user |
| Trivial mechanical change (rename, typo, formatting) | Lead acts directly; flow-plan is overhead |
| ~~`mode: minimal`~~ (LEGACY — modes eliminated 029/US12; /flow-lifecycle is always full) | Unreachable; a phase that doesn't apply is skipped with justification instead |
| Mid-implementation bug fix | Phase 3 `flow-build` + `troubleshooting` handle this |
...
For `flow-lifecycle`, set `tdd: forced` on behavior-changing HUs, including auxiliary code.
...
Ask the Phase 2 bank in `.claude/skills/drillme-clarify/references/03-phase-questions.md` §Phase 2 — that file is the single source, so the questions are not restated here — plus the canonical Socratic catalog, via the `drillme-clarify` skill (auxiliary — see "Auxiliary skills invoked" below). Its closing rule holds: any "I don't know" or evasive answer → iterate the plan; do NOT close.

> Skill-to-skill invocation is probabilistic. If `drillme-clarify` does not auto-fire, the Lead invokes `/drillme-clarify "Phase 2 plan closing for <NNN-slug>"` manually before approving hard gate 2->3.

### Step 8 — Stress-test alternatives (Full mode only)

If Full mode AND 2+ technically reasonable alternatives surfaced → invoke `compare-and-decide` (heavy tier) skill with the alternatives. Wait for verdict before closing the plan. Skip in Quick/Standard.
...
**Referencias verificadas**: cada HU referencia archivos/funciones/módulos verificados con Glob/Grep previo. Nunca inventar.
...
- **Execution-prompt rubric pass**: score each US's "Execution prompt (Phase 3 input)" block against `.claude/skills/prompt-design/scoring-criteria.md` (do not duplicate the rubric — Read it). A block scoring <70 gets refined BEFORE gate 2→3; the score is a doubt signal, not a hard stop.
...
### Step 13 — Invoke flow-test-plan (Phase 2.5)

Record phase 2 complete once draft tasks are ready. **Invoke `flow-test-plan` on these drafts** to produce `tests.md` or `validations.md` BEFORE returning control to user. Skill-to-skill invocation is probabilistic — the Lead verifies post-skill and dispatches manually if `flow-test-plan` auto-fire failed.

An existing oracle may be reused after checking its coverage; record that result for phase 2.5. A `/flow-lifecycle` task still needs an oracle before build.
...
Wiring and manual fallbacks for this phase (drillme-clarify, decide, prompt-design, harness-config, downstream flow-test-plan): `.claude/docs/auxiliary-skills-matrix.md` §Fallbacks per phase. Skill-to-skill invocation is probabilistic (issue #59968) — when an auxiliary does not fire, apply its fallback row. Step 13's `flow-test-plan` invocation is critical: verify post-skill that tests.md/validations.md exists; re-invoke manually if missing.
...
- ⚠️ Step 13 (`flow-test-plan` invocation) never produces tests.md/validations.md → skill-to-skill failed; Lead must invoke manually.
...
| Drillme bypass | Plan closes without running Phase 2 drillme-clarify | Hard gate internal before producing final tasks/ |
...
| VIII | Structured questionnaires + targeted research; delegation prompts reviewed by `prompt-design` |
...
- flow-test-plan invoked: yes|no (if no, Lead invokes manually)
