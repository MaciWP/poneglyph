...
| `drillme-clarify` | Step 7 — applies 5 retro-specific questions covering `[approach]`/`[context]`/`[failure]` | Lead invokes `/drillme-clarify "Phase 5 retro of <NNN-slug>"` manually before declaring the feature closed |
| `changes-explain` | ⚠️ Conditional — if the retro produces an educational walkthrough as a candidate promotion (e.g., a doc.md to onboard newcomers to this area) | Lead invokes `/changes-explain` manually if the candidate is a learning artefact |
| `harness-config` | ⚠️ Conditional — if a promotion candidate is native harness config (skill/rule/hook/agent/command/MCP/plugin/settings/permissions) | Lead Reads `.claude/skills/harness-config/SKILL.md` (and the matching type pack when present) before sketching the candidate |

> Skill-to-skill invocation is **probabilistic** per docs Anthropic + [issue #59968](https://github.com/anthropics/claude-code/issues/59968). Phase 5 is focused on synthesis — `drillme-clarify` is the canonical auxiliary; `changes-explain`/`harness-config` are conditional based on what the promotion candidates are. Honest 2 ✅ + 2 ⚠️.
...
- ⚠️ Living-spec deltas in >50% of retros → Phase 1 (scope) is too superficial; reopen `flow-scope` skill criteria.
...
- Smoke: invoke `/flow-retro` on a feature with `review.md: APPROVED` → produces `retro.md` with 8 sections + promotion candidates.
