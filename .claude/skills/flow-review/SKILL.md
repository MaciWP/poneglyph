...
name: flow-review
description: |
  Revisión end-to-end tras completar todas las HUs (Fase 4 del workflow `/flow-lifecycle`). Valida que el problema original de spec.md se resolvió de verdad. Produce review.md con checklist de 5 secciones (Correctness/Quality/Security/Performance/Maintainability) + findings con severidad + veredicto (APPROVED/WITH_WARNINGS/NEEDS_CHANGES/BLOCKED). Despacha UN reviewer de contexto fresco (read-only) y dispara security-audit en auth/pagos/credenciales.
  Úsala cuando: feature completo, todas las HUs cerradas en state.json, revisión antes de retro, tras /flow-build, "revisa", "critica", "valida", "audita", "veredicto".
metadata:
  keywords: >
...
| Trigger | Example |
|---|---|
| All HUs in `state.json.us_pending` are now in `state.json.us_completed` | After `/flow-build` closes the last HU |
| User invokes `/flow-review` explicitly | Direct invocation |
| `flow-build` skill closes Phase 3 and triggers Phase 4 invocation | Auto-chain from Phase 3 |
...
| Bug fix in non-poneglyph repo with no spec | Use `/flow-review` informally; skip the spec.md correctness check |
...
| Signal | Level | Scope |
|---|---|---|
| Feature trivial (1-2 HUs, no security/perf concern, mode minimal) | **light** | Base checks (tests + typecheck) + drillme-clarify Q1 only; skip fresh reviewer + code-quality + security-audit |
| Feature standard (3-N HUs, no critical area) | **standard** | Full 5-section checklist; code-quality quality mode; ONE fresh-context reviewer; the relevant canonical drillme-clarify bank |
| Feature architectural / touches auth/payments/secrets / complexity >60 | **full** | Standard + security-audit skill + code-quality both modes (quality + performance); fresh reviewer prompt additionally carries the critical-area focus |

CLI override: `/flow-review --light` / `--standard` / `--full`. Default = auto-detect from `spec.md.mode` + scanned content.
...
- Happy path E2E: simulate the user's actual flow — for diffs with a runtime surface, invoke `Skill(changes-verify)` (drive the affected flow end-to-end and observe behavior); manual walkthrough/smoke only where verify's anti-trigger applies (tests/docs/markdown-only — nothing to drive). (028/US5)
...
- **Lessons pass**: `Skill(lessons-learned)` — cross-repo guards + the `references/<stack>` file matching the diff. A violated lesson is a finding, quoted with its rule; G6 forbids APPROVED while a merge gate is red.

### Step 6 — Invoke `code-quality` skill catalog (AC8)

Per content of the diff:

| Diff signal | Mode to invoke | Path to Read |
|---|---|---|
| Refactoring / SOLID / DRY concerns / complexity | **quality mode** | `.claude/skills/code-quality/references/01-mode-quality.md` + subdirs in `quality/` |
| Loops with I/O / async / memory / hot paths | **performance mode** | `.claude/skills/code-quality/references/02-mode-performance.md` + subdirs in `performance/` |
...
**AC8 ratification**: `code-quality` is KEPT (17+ unique refs not duplicated here). The skill is the catalog; this skill is the orchestrator.
...
**Deliberative panels (≥4 perspectives)**: reserved for DECISION review — architecture choices, library selection, trade-off challenges — via `compare-and-decide` (heavy tier) (its evidence niche per 018 W1: cheap-to-grade decision tasks, not code). The critic does NOT launch panels for code.
...
`../drillme-clarify/references/03-phase-questions.md`. Sweep its relevant categories and
...
> Skill-to-skill invocation is **probabilistic**. If `drillme-clarify` does not auto-fire, the Lead invokes `/drillme-clarify "Phase 4 review of <NNN-slug>"` manually before declaring the verdict.
...
- code-quality modes: [<quality?>, <performance?>]
- security-audit invoked: <yes|no>
- drillme-clarify: covered 3/4 canonical Socratic categories

Next:
  → /flow-retro (if APPROVED or APPROVED_WITH_WARNINGS)
  → /flow-build US{N} (if NEEDS_CHANGES with specific HUs)
...
| Signal | Adaptation |
|---|---|
| Mode `light` (trivial feature, 1-2 HUs, no security/perf) | Base checks + drillme-clarify Q1 only; skip fresh reviewer + code-quality + security-audit |
| Mode `standard` (default, 3-N HUs, no critical area) | Full 5-section + code-quality quality + the relevant canonical drillme-clarify bank |
| Mode `full` (architectural / auth / payments / complexity >60) | Standard + security-audit + code-quality both modes; fresh reviewer carries critical-area focus |
| Doc-only feature (markdown changes, no code) | Quality + Maintainability sections only; skip Performance + Security; drillme-clarify Q1 |
| Bug fix with reproducible test | Correctness section + drillme-clarify Q3 (edge case); skip Performance unless bug was performance-related |
...
- **Edge 3** — `code-quality` no existe (cortada por error) → fallback al checklist embebido + flag in Issues; this should never happen (AC8 KEEP).
...
- ⚠️ A ≥4 deliberative panel launched for code review → doctrine regression (019 demotion, W1 D1/D3); panels belong to decision review via `compare-and-decide` (heavy tier) only.
...
| Panel launched for code review | Any ≥4-perspective Workflow fired from Phase 4 on a code diff | Replace with ONE fresh-context reviewer (correctness/requirements); panels = decisions only (`compare-and-decide` (heavy tier)) |
...
| Topic | File | Contents |
|---|---|---|
| Decisions history + auxiliary skills matrix | `references/01-decisions-and-auxiliaries.md` | Review-model evolution (reviewer agent → panel ≥4 → ONE fresh-context reviewer, feature 019), the AC8 code-quality KEEP rationale, the full auxiliary-skills invocation matrix with fallbacks, and post-implementation verification of this skill. Read when invoking auxiliaries beyond code-quality/security-audit, or when questioning the review model. |
...
Critical invariants kept in this body: `code-quality` is MANDATORY in standard/full (Step 6); `security-audit` is a MANDATORY gate on critical areas (Step 7); skill-to-skill auto-fire is probabilistic — the Lead dispatches manually on miss.
