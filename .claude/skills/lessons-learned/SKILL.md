...
| **G19 — Declare a missing spec in the FIRST delivery of the plan, not when challenged** | A ticket's Acceptance Criteria and References sections were empty and the linked design was unreachable; plan v1 read as fully specified, and the caveat only surfaced after the user rejected the plan asking "are you 100% sure?". Detailed evidence remains private. | When the requirement source is incomplete (empty ACs, unreachable design, undocumented endpoint), open the plan with what is **missing** and mark every inferred requirement as inferred. A plan that does not separate "the ticket asks this" from "I deduced this" hides its own risk, and the reader cannot audit what they cannot see |
| **G20 — Verbatim transcription is not a cheap-tier task** | A cheapest-tier agent told "do not summarise, copy every file path, code block and table verbatim" returned 20 lines for a 3-step story and 26 for an 8-item one. It reported the correct file list, so the structural check passed while fidelity failed. Detailed evidence remains private. | Faithfulness under length is a capability, not a mechanical chore — long verbatim copying belongs one tier up. When delegating it anyway, verify by **size against the source** (`wc -l` of output vs the section it copies), never by the agent's own file list. Extends G2: a report can satisfy the output format and still fail the requirement |
| **G21 — A git mutation never rides inside a measurement command** | A style-census one-liner embedded `git stash -q … git stash pop -q` behind `2>/dev/null`, unannounced, in a workspace whose stash stack is **shared across worktrees** — a pop conflict or a concurrent session would have mixed or lost work. Detailed evidence remains private. | Baselines come from `git show <ref>:<path>` — zero mutation. If state genuinely must move, the mutation is its **own declared step** (scoped `git stash push -m <tag> -- <paths>`, restore verified afterwards), never buried mid-pipeline behind `-q`/`2>/dev/null` |
| **G22 — A green publisher repo is not a compliant feature** | An API-contract change was approvable in its own repo while the consumer never called the new endpoint: it composed three legacy endpoints and recalculated client-side, and the generated types existed but were imported nowhere. Ten discrepancies, all invisible from the publisher repo. Detailed evidence remains private. | On a multi-repo ticket the AC-trace ends in the **consumer**: grep each consuming repo for the new endpoint/type actually imported and called. A review scoped to one repo declares that limit explicitly. The stack mechanics live in the private addon |
| **G23 — A validator only covers what its index can reach** | 2026-09-14: a memory file sat in the estate with no entry pointing at it — every session loaded the index and never saw the fact, while the estate looked healthy. The same shape bit a generated bundle elsewhere: an unregistered path passed lint, was silently omitted from the output, and no consumer ever received it. Detailed stack evidence remains private. | When a validator walks an index, writing the artefact is half the change — register it in the **same** commit. Verify against the *output* (grep the generated artefact, or the consumer's view) for the new name, never against the validator's green: an entry the index cannot reach is exactly what the validator has nothing to say about |
...
## G18 — Records and acknowledgements are not process identity or completion

The 2026-09-13 review found that `codex-brokers.ts` trusted a stored PID and
reported directory release before confirming shutdown. PID reuse can target an
unrelated process. Use the installed owner's state resolver and protocol; reject
invalid endpoints. An acknowledgement proves request acceptance, not process exit
or released resources. Local protocol fixtures cover rejection, silence and
bounded acknowledgement; they do not prove native lifecycle behavior.
