...
1. **Inventory BEFORE editing**: `grep -rn "<term/concept>" .claude/ CLAUDE.md --include="*.md" --include="*.ts" --include="*.js" --include="*.json" --include="*.toml" --include="*.yaml"` (exclude `plans/_archive`). List every hit: file:line + whether it states the doctrine (must change), cites it historically (legitimate — plans/, decision histories), or is unrelated.
...
|---|---|---|
| 2026-09-10 | Owner moved from `meta-create` to `meta-harness` (037/US1, AC29) | Plan `.claude/plans/037-meta-create-harness` |
