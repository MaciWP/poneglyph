...
parent: meta-harness
name: t02-command
description: T2 command pack — slash/dollar commands as an artifact type, not a second meta-harness entrypoint.
---
...
No `/meta-harness` command (D7). Commands are a type. Skills with `user-invocable: true` already appear as `/name` — prefer T1 unless you need a flat file with no supporting directory.
...
This repo: es-ES description. Do not add `/meta-harness`. Codex cell is `$name`, never “copy the Claude commands folder”.
...
| Companion `/meta-harness` | **ausente** (D7) |
