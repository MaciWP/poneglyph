...
For a feature, invoke `flow`; for bounded work, invoke `dev`. Do not replace their
...
2. Apply `orchestrator-protocol` and `drillme`. Split only real independent work.
...
- Review task prompts with `prompt-engineer`. Use the existing Arch H blocks with
...
Workers execute `dev` and the applicable `build` discipline. They return evidence;
...
`verify`. Failed or missing required checks leave the task unaccepted.
...
affected evidence. Use one approved fresh-context reviewer for `critic`; if no
