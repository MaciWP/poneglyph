...
// Quality review 2026-09-11, H44. prompt-design's Context 3 told the reviewer to
...
const TEMPLATE_REL = ".claude/skills/agent-routing/references/06-context-arch-h.md";
const template = readFileSync(resolve(root, TEMPLATE_REL), "utf8");
const skill = readFileSync(resolve(root, ".claude/skills/prompt-design/SKILL.md"), "utf8");
...
function checklistLine(): string {
  ...
  if (!line) throw new Error("no Arch H checklist line in prompt-design/SKILL.md");
  return line;
}
...
describe("prompt-design delegation checklist tracks the live Arch H template", () => {
