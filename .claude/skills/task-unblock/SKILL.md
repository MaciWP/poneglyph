...
metadata:
  keywords: >
    Keywords - task-unblock, desatascar, atascado, stuck, escalate, no se soluciona, loop, bucle,
...
Do NOT fire on a first failure — that's ordinary `troubleshooting` territory at base effort.
...
3. **Switch technique, don't repeat.** Load and apply a method you have NOT used yet:
   - `Skill('troubleshooting')` — 5 Whys, stack-trace classification, retry/recovery
   - `Skill('drillme-clarify')` — inversion / first-principles to surface a wrong assumption
   - `Skill('choose-skills')` — when the untried lever might be a skill you haven't reached for: propose→ratify a shortlist and apply the one not yet used (the change-of-attack IS often a change-of-tool)
...
- `troubleshooting` / `drillme-clarify` — the techniques this orchestrates
- `skills/agent-routing/references/09-loops-playbook.md` — escalation inside a goal-loop
