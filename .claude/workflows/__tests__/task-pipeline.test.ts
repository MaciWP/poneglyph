...
// Same contract-reproduction trick as flow-build-review.test.ts: a Workflow script is not an
...
const SOURCE = readFileSync(join(import.meta.dir, "..", "task-pipeline.js"), "utf8");
...
const REQUEST = {
  ...
  session_dir: "/tmp/task-pipeline/20260806T101500-abc1234",
  check_command: "bun test",
  baseline_path: "/tmp/task-pipeline/20260806T101500-abc1234/baseline.json",
  head: "abc1234",
  completed_phases: [],
};
const PLAN = { summary: "add a health endpoint", artifacts: ["/tmp/task-pipeline/x/plan.md"], steps: ["s1"], acceptance: ["a1"], commit_message: "spec: health endpoint" };
...
const DOC = { summary: "wrote it up", artifacts: ["/tmp/task-pipeline/x/document.md"], steps: [], commit_message: "docs: health endpoint" };
...
function harness(overrides: Record<string, unknown> = {}) {
  ...
  const agent = async (prompt: string, opts: any = {}) => {
    ...
    if (label.startsWith("gate") || label.startsWith("re-baseline")) return label.startsWith("re-baseline") ? { path: "/tmp/task-pipeline/x/baseline-rev1.json" } : GATE_OK;
