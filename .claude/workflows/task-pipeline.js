export const meta = {
  ...
  whenToUse:
    'Tarea acotada que hoy harías a mano porque montar un lifecycle /flow-lifecycle es desproporcionado, y quieres que el resultado quede verificado por código y no por la palabra del agente. Para features usa /flow-lifecycle (gates humanos); esto NO es /flow-lifecycle sin gates.',
  phases: [
...
if (!task) throw new Error('task-pipeline necesita args.task (p.ej. {"task":"añade un endpoint /health"})')
...
const run = await agent(
  `Set up an task-pipeline run. Do exactly this and return the structured object:\n` +
    `1) run_id: ${args?.runId ? `use EXACTLY "${args.runId}"` : 'run `date +%Y%m%dT%H%M%S` and append "-" plus `git rev-parse --short HEAD`'}.\n` +
    `2) session_dir: /tmp/task-pipeline/<run_id> — create it with mkdir -p. It MUST live OUTSIDE the repo: envelopes written inside the repo would show up as untracked changes and poison the very gate that reads them.\n` +
...
/** Normaliza un fallo para comparar iteraciones (mismo patrón que flow-build-review.js:174). */
...
function buildPrompt(previousViolations) {
  const base =
    ...
    `Do NOT git commit or push. Do NOT edit .claude/scripts/gate.ts or .claude/workflows/task-pipeline.js — a builder that edits its own grader invalidates the run.\n` +
